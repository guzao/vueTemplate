...
import { useWarningLevel } from '../hooks/useWarningLevel'
...
<template>
    <div class="mt-[8px] w-full overflow-hidden p-[24px] bg-[var(--theme-white-bg)]" v-loading="loading">
        ...
        <el-form :model="form" :rules="rules" class="demo-ruleForm mt-[36px] mb-[36px]" ref="formInstance" status-icon>
            ...
            <el-form-item label="告警等级" v-loading="!dicts.warningLevelDict.loaded" prop="levels">

                <el-checkbox v-model="checkAll" :indeterminate="isIndeterminate" @change="handleCheckAllChange"
                    class="pr-4">{{ t('common.all') }}</el-checkbox>

                <el-checkbox-group v-model="form.levels" @change="handleCheckedIdsChange">
                    <el-checkbox v-for="item in dicts.warningLevelDict.dictValue" :label="item.dictValue"
                        :key="item.dictValue">
                        {{ item.dictLabel }}
                    </el-checkbox>
                </el-checkbox-group>
